 ...
 */
function gavias_profile_install_tasks_alter(&$tasks, &$install_state) {
  $new_tasks = array(
...
function tb_install_selected_profile() {
  ...
  if ($driver == 'mysql' || $driver == 'mysqli') {
    ...
    $link = mysqli_connect($sqlserver,$user,$password,$database) or die("Error " . mysqli_error($link)); 
    ...
    $success = tb_import_database($file_name, $prefix, $link);
